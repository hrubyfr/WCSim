...
namespace {
  struct SecondaryPhotonPEInfo {
    int   pmt = -1;
    float time_ns = 0.f;
    float lambda_nm = 0.f;
  };

  static std::unordered_map<long long, SecondaryPhotonPEInfo> g_secondaryPhotonPE;
  static int g_secondaryPhotonPE_evt = -999999;

  long long MakeKey(int evt, int trk)
  {
    const auto evtBits = static_cast<unsigned int>(evt);
    const auto trkBits = static_cast<unsigned int>(trk);
    const auto key = (static_cast<unsigned long long>(evtBits) << 32) | trkBits;
...
void AllSecondaryPhotonsTree_Book(TFile* fout)
{
  ...
  all_secondary_photons_tree = new TTree(
      "AllSecondaryPhotons",
      "Cherenkov photons produced by the event primary and all non-optical secondary particles");
...
{
  if (evt == g_secondaryPhotonPE_evt) return;
  g_secondaryPhotonPE_evt = evt;
  g_secondaryPhotonPE.clear();
}

void AllSecondaryPhotonsTree_NotePE(int evt, int photon_trk, int pe_pmt,
                                    float pe_time_ns, float pe_lambda_nm)
{
  AllSecondaryPhotonsTree_ResetEvent(evt);
  SecondaryPhotonPEInfo info;
  info.pmt = pe_pmt;
  info.time_ns = pe_time_ns;
  info.lambda_nm = pe_lambda_nm;
  g_secondaryPhotonPE[MakeKey(evt, photon_trk)] = info;
}
...
{
  AllSecondaryPhotonsTree_ResetEvent(evt);
  auto it = g_secondaryPhotonPE.find(MakeKey(evt, photon_trk));
  if (it == g_secondaryPhotonPE.end()) return false;

  pe_pmt = it->second.pmt;
  pe_time_ns = it->second.time_ns;
  pe_lambda_nm = it->second.lambda_nm;
  g_secondaryPhotonPE.erase(it);
  return true;
}
